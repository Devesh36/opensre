...
Set `MONGODB_TLS=true` (default) in `.env` or the integration store. TLS options beyond that are encoded in the connection string (for example Atlas `mongodb+srv://…`).
...
| **SSL: CERTIFICATE_VERIFY_FAILED** | Fix trust in the connection string or host CA bundle; test with `mongosh` using the same URI. |

...
- Completion notifications can go to email ([SMTP](/smtp)), Telegram
  ...
  ([Rocket.Chat](/messaging/rocketchat)), or any combination of those
...
| `/background notify set <channel[,channel...]>` | Set completion channels — `email`, `telegram`, `rocketchat`, or a comma-separated list |
...
Those stats are included in notification messages (email, Telegram, Rocket.Chat)
...
**What to do next**, plus a short internal stats section. Telegram and
Rocket.Chat messages are plain text and are capped at the 4,096-character

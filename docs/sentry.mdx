...
Deliver a daily unresolved-issues summary to Slack or Telegram. The Issues
section is built deterministically from Sentry (`search_sentry_issues` data
shape via `build_sentry_issue_digest`), not an LLM instruction block. An optional
uptime section is appended from the uptime watch transition log when history
exists. This is not the investigation pipeline or generic `opensre cron` kinds.
...
When an **uptime watch** schedule is running, the digest may also include a
deterministic **Uptime / downtime (last 24h)** section built from the watch
transition log (monitors that went down, recovered, or are still down). That
section is separate from the LLM Issues summary and does not use the
`sentry-summary` skill. If no uptime watch history exists, the digest stays
Issues-only.

The gateway daemon picks up scheduled digests automatically when it is running.
Issues and uptime sections are deterministic; no LLM is required for delivery.
...
Schedule an uptime watch alongside the morning digest if you want the digest to
include yesterday's downtime summary; the watch records DOWN/RECOVERED
transitions locally for that rollup.

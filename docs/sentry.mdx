...
headless **sentry-summary** skill path (tools + LLM answer via the gateway),
not the investigation pipeline or generic `opensre cron` kinds. When an uptime
watch schedule is running, the skill also calls `get_sentry_uptime_digest` to
include downtime/recovery context in the digest.

...
<Info>
  **Alpha execution policy:** slash commands, investigations, shell one-liners, and CLI parity actions run immediately with **no confirmation prompts**. `/trust` remains registered for a future stricter policy but does not gate commands today.
</Info>
...
**TTY vs non-TTY:** Interactive menus and onboarding wizards work best in a real terminal. Piping input or running in CI sets non-interactive mode — prefer explicit `opensre …` CLI commands outside the REPL for automation. Under the alpha policy, actions are not blocked for lack of a confirmation TTY.
...
| `/save <path>` | Export the last investigation to a file |
| `/rca` | Browse persisted RCA reports across sessions |
...
| Command | What it does |
| --- | --- |
| `/watch` | Watch a process and send Telegram threshold alarms |
| `/watches` | List active watchdog tasks with latest samples |
| `/unwatch <id>` | Stop a watchdog task by id |
...
| `/remote` | CLI parity for remote workflows (see [Remote runtime investigation](/remote-runtime-investigation)) |
| `/gateway` | Start, stop, status, or tail logs for the gateway daemon |
| `/misses` | List, export, or convert recorded action misses |
...
| `/uninstall` | Remove OpenSRE and local data |
...
Trust mode is a **session preference** retained for a future confirmation policy. In alpha, slash and shell actions run without prompts regardless of trust state.
...
In a TTY, bare `/trust` opens an interactive on/off menu. Trust mode is not restored by `/resume`.
...
Forwards to `opensre remote …` (not registered on the current top-level CLI). For deployed runtimes, use [Deployment](/deployment) and `opensre investigate -i` with an alert payload. See [Remote runtime investigation](/remote-runtime-investigation).
...
During alpha, the REPL uses an **allow-all** execution policy: no `[y/N]` confirmation prompts for slash commands, investigations, or shell input. `/trust` still toggles a session flag for compatibility and for a possible post-alpha confirmation tier.

Non-TTY sessions are not blocked solely because stdin is not interactive; use headless CLI (`opensre investigate …`) when automation must avoid the REPL entirely.
...
When the planner is uncertain, it asks for clarification or falls back to help.

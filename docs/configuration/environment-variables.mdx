...
| Variable | Default | Description | Module |
|----------|---------|-------------|--------|
| `OPENSRE_REASONING_EFFORT` | `medium` | Reasoning effort for extended-thinking models (`low`, `medium`, `high`, `xhigh`; REPL `/effort` on OpenAI/Codex also supports `max`) | `llm_reasoning_effort` |
...
| `TRACER_OUTPUT_FORMAT` | | Output format for tracer results | `platform/observability/render/output_format.py` |
| `TRACER_API_URL` | | API URL for tracer reports | `integrations/slack/delivery.py` |
| `NO_COLOR` | | Disable colored output (standard convention) | `platform/observability/render/output_format.py` |
...
| Variable | Default | Description | Module |
|----------|---------|-------------|--------|
| `OPENSRE_NO_TELEMETRY` | `0` | Disable all telemetry, including Sentry and PostHog (covers `$ai_generation` prompt/response events) | `platform/observability/errors/sentry.py` |
| `OPENSRE_SENTRY_DISABLED` | `0` | Disable Sentry error reporting | `platform/observability/errors/sentry.py` |
| `OPENSRE_SENTRY_DSN` | | Override Sentry DSN | `platform/observability/errors/sentry.py` |
...
| `DO_NOT_TRACK` | `0` | Honor global do-not-track preference | `platform/observability/errors/sentry.py` |
...
| `GITLAB_ACCESS_TOKEN` | | GitLab API token | `.env.example` |
...
| `INCIDENT_IO_API_KEY` | | incident.io API token | `.env.example` |

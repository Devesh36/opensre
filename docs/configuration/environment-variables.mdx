...
| Variable | Default | Description |
| --- | --- | --- |
| `TRACER_VERBOSE` | `0` | Verbose output for tracer (`1`, `true`, `yes`) |
| `TRACER_OUTPUT_FORMAT` |  | Output format for tracer results |
| `TRACER_API_URL` |  | API URL for tracer reports |
| `JWT_TOKEN` |  | Tracer JWT for hosted / web integrations |
...
2. The OS keychain (Keychain on macOS, etc.)
3. A local fallback file if there's no keychain (see below)

`opensre onboard` often saves credentials so they still work after you clear `.env`.

**Not stored in the keychain:** webhook-style URLs such as `SLACK_WEBHOOK_URL` and `ROCKETCHAT_WEBHOOK_URL`. Those stay in env / store only — treat them as secrets and don't log them.

Want credentials only from the environment, never on disk? Set `OPENSRE_DISABLE_KEYRING=1` and export everything yourself.

### No system keychain?

Headless Linux, SSH on EC2, Docker, and CI often have no keychain. A macOS Keychain can also be locked. In those cases OpenSRE writes to `~/.opensre/credentials.json` with owner-only permissions (`0600`) and tells you. Unlock or install a keychain later and re-run setup to move secrets back into secure storage.

| Variable | Default | Description |
| --- | --- | --- |
| `OPENSRE_DISABLE_KEYRING` | `0` | Disable local credential storage; credentials must come from the environment |
| `OPENSRE_USE_KEYRING` | `0` | Opt-in keyring writes (`1` to store new secrets in the OS keyring) |
| `OPENSRE_LLM_AUTH_METADATA_PATH` | `~/.opensre/llm-auth.json` | Non-secret LLM auth metadata path used by prompt-safe status checks |
| `DBUS_SESSION_BUS_ADDRESS` |  | D-Bus socket address (Linux keyring) |
...
| `OPENSRE_REMOTE_SYNC_PROVIDER` | `aws` | Store backend (`aws` or `vercel`) |
...
| `VERCEL_RUNTIME_LOGS_READ_TIMEOUT` |  | Timeout for Vercel runtime log reads |

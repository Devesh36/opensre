...
- **Task definitions** are stored in `~/.opensre/scheduler_tasks.json`
  (JSON + filelock)
- **Execution history** is stored in `~/.opensre/scheduler.db` (SQLite with
  WAL mode)
- Both survive process restarts — `opensre cron list` and `opensre cron logs`
  read from disk

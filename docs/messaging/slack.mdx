...
Wire morning digests, PR sweeps, and `/loops` prompt loops to Slack. The gateway
daemon (or `opensre cron start`) must be running so the scheduler can deliver
when a tick finishes — the report arrives proactively; you do not need to ask
in chat.
...
Copy the channel id (`C…`) from Slack channel details. During `opensre integrations setup slack` (Socket Mode or Both), you can persist a default channel id for scheduled delivery.

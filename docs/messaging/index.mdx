...
Existing values are pre-populated, so you can press Enter to keep what you have. Use `opensre integrations setup slack`, `opensre integrations setup whatsapp`, `opensre integrations setup twilio`, or `opensre integrations setup telegram` (or `/integrations setup …` in the REPL). See each channel page for env-var-only setup.

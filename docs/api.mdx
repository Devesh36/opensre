...
OpenSRE exposes a single FastAPI application (`gateway/http/webapp.py`) on one
port (default `8000`). Use these endpoints to invoke OpenSRE over the network.
For in-process use from Python, see the [Python API](/python-api).

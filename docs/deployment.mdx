...
deduplication escape hatch. Conversation sessions and scheduler state remain
file-backed under `OPENSRE_HOME`; use a shared mount before running multiple
processes that need the same files. See [HTTP API](/api) for the route
reference.

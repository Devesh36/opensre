...
`@live` gather scenarios **fail** (not skip) in GitHub Actions when integration credentials are missing; locally they may still skip. Natural-language investigation dispatch scenarios run in the sharded `turn-live` job alongside other live REPL turns. Require all `turn-checks` and `turn-live shard *` checks on `main` branch protection.

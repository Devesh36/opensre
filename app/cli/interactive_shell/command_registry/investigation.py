...
def _cmd_investigate_file(session: ReplSession, console: Console, args: list[str]) -> bool:
    ...
    if template_name:
        ...
        session.apply_investigation_result(final_state, trigger=f"/investigate {template_name}")
    ...
    root = final_state.get("root_cause")
    task.mark_completed(result=str(root) if root is not None else "")
    session.apply_investigation_result(final_state, trigger=f"/investigate {raw_target}")
